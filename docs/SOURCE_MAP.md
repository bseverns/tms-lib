...
## Notes
- `silt` was inspected for this pass, but `Plate`, `CombBank`, and `ScaleMap` are still skeleton stubs there, so there was not yet real source logic worth adapting into `tms-lib`.
